...
impl Agent for SolutionAgent2 {
    ...
    fn solve(board: &mut Board, player: Player, _time_limit: u64) -> (i32, usize, usize) {
        ...
        if moves.len() == 7 && (matches!(player, Player::X) && cells[1][1] == Cell::X || matches!(player, Player::O) && cells[1][1] == Cell::O) {
            ...
            if let Some(edge) = edges.iter().find(|edge| !moves.contains(edge)) { // I used claude to figure out this syntax of using find and the |   | to see if a vector contains a specific value
            ...
            } else { //if 2, I need to take the corner on the other end of the diagnol
        ...
        if moves.len() == 6 && (matches!(player, Player::X) && cells[1][1] == Cell::O || matches!(player, Player::O) && cells[1][1] == Cell::X) {
            //if I don't have center cell <- necessary for us to do turn 6
            if let Some(corner) = corners.iter().filter(|corner| **corner != (0,0)).find(|corner| !moves.contains(corner)) {
        ...
        for &(r, c) in &moves { // index each possible board space
            board.apply_move((r, c), player); //apply a move there 
            if board.game_over() { //if this wins the board, then we just return that 
